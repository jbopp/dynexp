...
In turn, the abstraction of instruments enables writing hardware-agnostic programs to control and automate experiments, so-called modules.
...
- *EventSender*: Allows issuing inter-module events manually.
...
- *LaserControl*: Allows controlling a *Laser* instrument.
- *LockinAmplifierControl*: Allows controlling a *LockinAmplifier* instrument.
...
- *Stage1D*: Allows controlling a single positioner *Stage* instrument.
...
- Experiments
	- *LaserScanningSpectroscopy*: Allows sweeping the frequency of a *Laser* instrument while recording data for each frequency. For instance, data can be acquired by connected *ImageViewer* or *StreamManipulator* modules, enabling (widefield) photoluminescence excitation (PLE) measurements.
	- *ODMR*: Allows recording ODMR spectra with a *LockinAmplifier* or *DataStreamInstrument* by sweeping RF signals generated by a *FunctionGenerator*.
	- *WidefieldMicroscope*: Implements a reflected-light, a wide-field, and a confocal microscope using a *Camera* and multiple *Stage* instruments. Allows recording photoluminescence (PL) scans. Furthermore, this module provides access to a *TimeTagger*'s HBT functionality and features for automated characterization of single-photon emitters spread across a sample's surface.
...
A *SignalDesigner* module enables adjusting the temperature setpoint.
